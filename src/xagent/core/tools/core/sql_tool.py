...
Example:
    ...
    XAGENT_EXTERNAL_DB_FLIGHT_DORIS=mysql+pymysql://user:pass@127.0.0.1:9030/xagent
    XAGENT_EXTERNAL_DB_FLIGHT_DORIS=mysql+pymysql://root:@127.0.0.1:9030/xagent  # Doris, no password

Apache Doris uses the MySQL protocol (FE query port often 9030); use mysql+pymysql and a
dedicated NAME (e.g. FLIGHT_DORIS) so other XAGENT_EXTERNAL_DB_* connections are unaffected.
