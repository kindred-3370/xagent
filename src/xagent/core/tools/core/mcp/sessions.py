...
async def _create_websocket_session(
    *,
    url: str,
    headers: dict[str, Any] | None = None,  # Not used
    session_kwargs: dict[str, Any] | None = None,
) -> AsyncIterator[ClientSession]:
...
async def create_session(connection: Connection) -> AsyncIterator[ClientSession]:  # noqa: C901
    ...
    # Filter out non-parameter fields like 'name', 'description' etc.
    valid_params = {
        "command",
        "args",
        "env",
    ...
    params = {
        k: v for k, v in connection.items() if k != "transport" and k in valid_params
    }
    if transport == "sse":
        ...
        async with _create_sse_session(**params) as session:  # type: ignore
            yield session
    elif transport == "streamable_http":
        if "url" not in params:
            msg = "'url' parameter is required for Streamable HTTP connection"
            raise ValueError(msg)
        async with _create_streamable_http_session(**params) as session:  # type: ignore
            yield session
    elif transport == "stdio":
        if "command" not in params:
            msg = "'command' parameter is required for stdio connection"
            raise ValueError(msg)
        async with _create_stdio_session(**params) as session:  # type: ignore
            yield session
    elif transport == "websocket":
        if "url" not in params:
            msg = "'url' parameter is required for Websocket connection"
            raise ValueError(msg)
        async with _create_websocket_session(**params) as session:  # type: ignore
            yield session
    else:
        msg = (
