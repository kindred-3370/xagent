...
import { FlightDailyLineChart } from "@/components/charts/FlightDailyLineChart";
import { extractFlightDailyStatsFromTraceEvents } from "@/lib/flight-daily-stats";
...
export function ChatMessage({
  ...
  const flightDailyChartPayload = useMemo(() => {
    if (isUser || !Array.isArray(traceEvents) || traceEvents.length === 0) {
      return null;
    }
    return extractFlightDailyStatsFromTraceEvents(traceEvents);
  }, [isUser, traceEvents]);
  ...
  return (
    <div className="w-full space-y-2 animate-fade-in group">
      ...
      {!isUser && flightDailyChartPayload && (
        <div className={cn("pl-7")}>
          <FlightDailyLineChart payload={flightDailyChartPayload} />
        </div>
      )}
